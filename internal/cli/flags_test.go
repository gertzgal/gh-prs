...
func TestComposeFlags_DebugFromFlag(t *testing.T) {
	got := composeFlags("", true, false, "", false, nil, nil, nil, map[string]string{})
	if !got.Debug {
...
func TestComposeFlags_DebugFromEnv(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"DEBUG": "1"})
	if !got.Debug {
...
func TestComposeFlags_DebugEnvEmpty(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"DEBUG": ""})
	if got.Debug {
...
func TestComposeFlags_NoCacheFromFlag(t *testing.T) {
	got := composeFlags("", false, true, "", false, nil, nil, nil, map[string]string{})
	if !got.NoCache {
...
func TestComposeFlags_NoCacheFromEnv(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_NO_CACHE": "1"})
	if !got.NoCache {
...
func TestComposeFlags_NoCacheEnvFalsy(t *testing.T) {
	for _, v := range []string{"", "0", "false", "False"} {
		got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_NO_CACHE": v})
		if got.NoCache {
...
func TestComposeFlags_CacheTTLFlagWins(t *testing.T) {
	got := composeFlags("", false, false, "2m", false, nil, nil, nil, map[string]string{"GH_PRS_CACHE_TTL": "5m"})
	if got.CacheTTL != "2m" {
...
func TestComposeFlags_CacheTTLFromEnv(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_CACHE_TTL": "5m"})
	if got.CacheTTL != "5m" {
...
func TestComposeFlags_FormatFromFlag(t *testing.T) {
	for _, name := range []string{render.FormatText, render.FormatJSON, render.FormatTOON} {
		got := composeFlags(name, false, false, "", false, nil, nil, nil, map[string]string{})
		if got.Format != name {
...
func TestComposeFlags_FormatFromEnv(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_FORMAT": "toon"})
...
func TestComposeFlags_FormatFlagWinsOverEnv(t *testing.T) {
	got := composeFlags("json", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_FORMAT": "toon"})
...
func TestComposeFlags_FormatCaseInsensitive(t *testing.T) {
	got := composeFlags("  JSON  ", false, false, "", false, nil, nil, nil, map[string]string{})
	if got.Format != render.FormatJSON {
...
func TestComposeFlags_StatsFromFlag(t *testing.T) {
	got := composeFlags("", false, false, "", true, nil, nil, nil, map[string]string{})
	if !got.Stats {
...
func TestComposeFlags_StatsFromEnv(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_STATS": "1"})
	if !got.Stats {
...
func TestComposeFlags_StatsEnvFalsy(t *testing.T) {
	for _, v := range []string{"", "0", "false", "False"} {
		got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_STATS": v})
		if got.Stats {
...
func TestComposeFlags_AuthorFromFlag_Single(t *testing.T) {
	got := composeFlags("", false, false, "", false, []string{"alice"}, nil, nil, map[string]string{})
...
func TestComposeFlags_AuthorFromFlag_Multiple(t *testing.T) {
	got := composeFlags("", false, false, "", false, []string{"alice", "bob"}, nil, nil, map[string]string{})
...
func TestComposeFlags_AuthorFromEnv_Single(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_AUTHOR": "alice"})
	if len(got.Authors) != 1 || got.Authors[0] != "alice" {
...
func TestComposeFlags_AuthorFromEnv_CommaSeparated(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_AUTHOR": "alice,bob"})
	if len(got.Authors) != 2 || got.Authors[0] != "alice" || got.Authors[1] != "bob" {
...
func TestComposeFlags_AuthorFlagWinsOverEnv(t *testing.T) {
	...
	got := composeFlags("", false, false, "", false, []string{"carol"}, nil, nil, map[string]string{"GH_PRS_AUTHOR": "alice,bob"})
...
func TestComposeFlags_AuthorEnvEmpty_ProducesNil(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_AUTHOR": ""})
	if len(got.Authors) != 0 {
...
// ---------------------------------------------------------------------------
// --exclude-author / GH_PRS_EXCLUDE_AUTHOR
// ---------------------------------------------------------------------------

func TestComposeFlags_ExcludeAuthorDefault_NilWhenNotSet(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{})
	if len(got.ExcludeAuthors) != 0 {
		t.Errorf("ExcludeAuthors: want empty, got %v", got.ExcludeAuthors)
	}
}

func TestComposeFlags_ExcludeAuthorFromFlag_Single(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, []string{"alice"}, map[string]string{})
	if len(got.ExcludeAuthors) != 1 || got.ExcludeAuthors[0] != "alice" {
		t.Errorf("ExcludeAuthors: want [alice], got %v", got.ExcludeAuthors)
	}
}

func TestComposeFlags_ExcludeAuthorFromFlag_Multiple(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, []string{"alice", "bob"}, map[string]string{})
	if len(got.ExcludeAuthors) != 2 || got.ExcludeAuthors[0] != "alice" || got.ExcludeAuthors[1] != "bob" {
		t.Errorf("ExcludeAuthors: want [alice bob], got %v", got.ExcludeAuthors)
	}
}

func TestComposeFlags_ExcludeAuthorFromEnv_CommaSeparated(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_EXCLUDE_AUTHOR": "alice,bob"})
	if len(got.ExcludeAuthors) != 2 || got.ExcludeAuthors[0] != "alice" || got.ExcludeAuthors[1] != "bob" {
		t.Errorf("ExcludeAuthors: want [alice bob] from env, got %v", got.ExcludeAuthors)
	}
}

func TestComposeFlags_ExcludeAuthorFromEnv_TrimsSpaces(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_EXCLUDE_AUTHOR": " alice , bob "})
	if len(got.ExcludeAuthors) != 2 || got.ExcludeAuthors[0] != "alice" || got.ExcludeAuthors[1] != "bob" {
		t.Errorf("ExcludeAuthors: want trimmed [alice bob], got %v", got.ExcludeAuthors)
	}
}

func TestComposeFlags_ExcludeAuthorFlagWinsOverEnv(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, []string{"carol"}, map[string]string{"GH_PRS_EXCLUDE_AUTHOR": "alice,bob"})
	if len(got.ExcludeAuthors) != 1 || got.ExcludeAuthors[0] != "carol" {
		t.Errorf("ExcludeAuthors: flag should win over env, want [carol], got %v", got.ExcludeAuthors)
	}
}

func TestComposeFlags_ExcludeAuthorEnvEmpty_ProducesNil(t *testing.T) {
	got := composeFlags("", false, false, "", false, nil, nil, nil, map[string]string{"GH_PRS_EXCLUDE_AUTHOR": ""})
	if len(got.ExcludeAuthors) != 0 {
		t.Errorf("ExcludeAuthors: empty env should produce no excludes, got %v", got.ExcludeAuthors)
	}
}
