...
func TestComposeFlags_CacheTTLFlagWins(t *testing.T) {
	got := composeFlags("", false, false, "2m", false, map[string]string{"GH_PRS_CACHE_TTL": "5m"})
	if got.CacheTTL != "2m" {
...
func TestComposeFlags_StatsFromEnv(t *testing.T) {
	got := composeFlags("", false, false, "", false, map[string]string{"GH_PRS_STATS": "1"})
	if !got.Stats {
...
func TestComposeFlags_StatsEnvFalsy(t *testing.T) {
	for _, v := range []string{"", "0", "false", "False"} {
		got := composeFlags("", false, false, "", false, map[string]string{"GH_PRS_STATS": v})
		if got.Stats {
